...
func TestDCPV2Stage6DirectAdoptValidatesBeforeOpeningStore(t *testing.T) {
	dataDir := filepath.Join(t.TempDir(), "data")
	t.Setenv("AO_DATA_DIR", dataDir)
	t.Setenv("AO_RUN_FILE", filepath.Join(t.TempDir(), "running.json"))
	cmd := NewRootCommand(Deps{In: bytes.NewReader(nil), Out: &bytes.Buffer{}, Err: &bytes.Buffer{}})
	cmd.SetArgs([]string{"dcp", "stage6-direct-adopt", "--source-commit", "bad",
		"--source-tree", strings.Repeat("b", 40), "--install-receipt-sha", strings.Repeat("c", 64)})
	if err := cmd.Execute(); err == nil {
		t.Fatal("invalid direct-adoption source identity unexpectedly succeeded")
	}
	if _, err := os.Stat(filepath.Join(dataDir, "ao.db")); !os.IsNotExist(err) {
		t.Fatalf("invalid direct-adoption input opened the database: %v", err)
	}
}
