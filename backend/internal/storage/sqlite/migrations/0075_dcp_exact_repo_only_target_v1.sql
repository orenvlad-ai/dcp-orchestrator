...
-- +goose Up
-- Widen only the existing future-policy identity table to the second exact
-- compile-time target. All mutable state, action slots, reviews, incidents and
-- admissions continue to use their existing durable authorities.
PRAGMA foreign_keys = OFF;

CREATE TABLE dcp_review_lab_policy_task_v75 (
    ...
    CHECK (
      ...
      (target = 'wb-price-extension' AND profile = 'repo-only' AND
       ...
       policy_version = 'dcp.repo-only.happy-path/v1')
    ),
...
INSERT INTO dcp_review_lab_policy_task_v75
...
ALTER TABLE dcp_review_lab_policy_task_v75 RENAME TO dcp_review_lab_policy_task;
