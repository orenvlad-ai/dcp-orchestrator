...
func TestDCPFutureArbiterOneCallRepairFreshReviewMergeAndRestartDedupe(t *testing.T) {
	...
	if ok, err := s.RecordDCPFutureArbiterDecision(ctx, incident, claimed, decisionJSON, digest('6'), domain.DCPFutureVerdictRepair,
		`["arb-store-a"]`, "Rebase the compatible intent onto exact current main.", `["shared.txt"]`, "", now.Add(8*time.Second)); err != nil || !ok {
		t.Fatalf("record repair decision = %v, %v", ok, err)
	}
	repair, ok, err := s.ClaimNextDCPModelAction(ctx, now.Add(9*time.Second))
	...
	if ok, err := s.StartDCPModelAction(ctx, repair, "repair-generation", "", now.Add(10*time.Second)); err != nil || !ok {
		t.Fatalf("start repair = %v, %v", ok, err)
	}
	task, _, _ = s.GetDCPReviewLabPolicyTaskByTaskID(ctx, task.TaskID)
	task.State = domain.DCPPolicyCIWaiting
	if ok, err := s.FinishDCPModelAction(ctx, repair, task, domain.DCPActionSucceeded, "", now.Add(11*time.Second)); err != nil || !ok {
		t.Fatalf("finish repair = %v, %v", ok, err)
	}

	newHead := strings.Repeat("c", 40)
	newBase := incident.CurrentMainSHA
	newRun := seedArbiterRecoveryReview(t, s, incidentAdmission, newHead, newBase, now.Add(12*time.Second))
	task, _, _ = s.GetDCPReviewLabPolicyTaskByTaskID(ctx, task.TaskID)
	reviewAction := domain.DCPModelAction{ID: "dcp-model-arb-store-a-review-2", TaskID: task.TaskID, SessionID: task.SessionID,
		Kind: domain.DCPActionReviewer, ExactHeadSHA: newHead, Status: domain.DCPActionQueued, CreatedAt: now.Add(13 * time.Second), UpdatedAt: now.Add(13 * time.Second)}
	reviewQueued := task
	...
	review, ok, err := s.ClaimNextDCPModelAction(ctx, now.Add(14*time.Second))
	if err != nil || !ok || review.ID != reviewAction.ID {
		t.Fatalf("claim fresh review = %+v, %v, %v", review, ok, err)
	}
	if ok, err := s.StartDCPModelAction(ctx, review, "fresh-review-handle", newRun.ID, now.Add(15*time.Second)); err != nil || !ok {
	...
	if ok, err := s.FinishDCPModelAction(ctx, review, bound, domain.DCPActionSucceeded, "", now.Add(16*time.Second)); err != nil || !ok {
		t.Fatalf("finish fresh review = %v, %v", ok, err)
	}
	bound, _, _ = s.GetDCPReviewLabPolicyTaskByTaskID(ctx, task.TaskID)
	if ok, err := s.RebindDCPFutureArbiterAdmission(ctx, incident, bound, newRun, newBase, now.Add(17*time.Second)); err != nil || !ok {
	...
	if ok, err := s.ClaimDCPReviewLabAdmission(ctx, rebound, "merge-"+admission.ID, newBase, now.Add(18*time.Second)); err != nil || !ok {
		t.Fatalf("claim rebound = %v, %v", ok, err)
	}
	rebound, _, _ = s.GetClaimedDCPReviewLabAdmission(ctx)
	bound, _, _ = s.GetDCPReviewLabPolicyTaskByTaskID(ctx, task.TaskID)
	mergeSHA := strings.Repeat("d", 40)
	if ok, err := s.CompleteDCPReviewLabPolicyAdmission(ctx, rebound, bound, mergeSHA, now.Add(19*time.Second)); err != nil || !ok {
	...
	if counts[domain.DCPActionArbiter] != 1 || counts[domain.DCPActionRepairWorker] != 1 || counts[domain.DCPActionReviewer] != 1 {
