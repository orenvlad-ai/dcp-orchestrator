...
source_gates() {
	...
	future_arbiter_schema_recovery_migration='backend/internal/storage/sqlite/migrations/0070_dcp_future_arbiter_schema_recovery.sql'
	future_arbiter_result_recovery_migration='backend/internal/storage/sqlite/migrations/0071_dcp_future_arbiter_result_validation_recovery.sql'
	future_repair_target_recovery_migration='backend/internal/storage/sqlite/migrations/0072_dcp_future_repair_target_validation_recovery.sql'
	future_repair_ci_recovery_migration='backend/internal/storage/sqlite/migrations/0073_dcp_future_repair_ci_snapshot_recovery.sql'
	future_human_gate_recovery_migration='backend/internal/storage/sqlite/migrations/0074_dcp_future_human_gate_result_recovery.sql'
	...
	[[ -s "$provider_pending_recovery_migration" ]] || fail 'exact provider-pending recovery migration is absent'
	...
	! sed -n '/func FutureArbiterDecisionJSONSchema/,/func validateFutureArbiterResponseSchema/p' backend/internal/dcpterminalmerge/future_arbiter_protocol.go | grep -Eq '"(oneOf|anyOf|allOf|not|const|uniqueItems|\$schema)"[[:space:]]*:' || fail 'ordinary-card response schema uses unsupported composition'
	...
	[[ -s "$repo_only_target_migration" ]] || fail 'exact repo-only target migration is absent'
	...
	[[ -s "$real_target_submit_recovery_migration" ]] || fail 'exact real-target submit recovery migration is absent'
	grep -Fq "task_id = 'price-arch-v1'" "$real_target_submit_recovery_migration" || fail 'real-target submit recovery task is not exact'
	grep -Fq "reviewer_action_sequence = 60" "$real_target_submit_recovery_migration" || fail 'real-target reviewer accounting sequence is not exact'
	grep -Fq 'worker_token_count = 27373' "$real_target_submit_recovery_migration" || fail 'real-target worker token accounting is absent'
	grep -Fq 'reviewer_token_count = 20512' "$real_target_submit_recovery_migration" || fail 'real-target reviewer token accounting is absent'
	[[ -s "$repo_only_target_forward_migration" ]] || fail 'repo-only forward target migration is absent'
	grep -Fq "target = 'wb-browser-extension'" "$repo_only_target_forward_migration" || fail 'current repo-only target is not physical'
	...
	grep -Fq '"gh", "api", "--method", "GET", "repos/"+repository' backend/internal/dcpterminalmerge/merge.go || fail 'terminal merge provider lookup does not use stable REST repository metadata'
	...
	grep -Fq 'showReview = reviewSessions.length > 0' frontend/src/renderer/components/SessionsBoard.tsx || fail 'empty review subsection is still rendered'
	...
	[[ -s "$future_human_gate_recovery_migration" ]] || fail 'exact ordinary-card HumanGate result recovery migration is absent'
	grep -Fq '98e4d77336bdfc1539aa44932eacc45514adcbbf8600ff0483d0fb1fb1ed499a' "$future_human_gate_recovery_migration" || fail 'HumanGate recovery incident identity is not exact'
	grep -Fq '46c44780cee9a9f198e854890448400c9c208d920233f1acba399cadb5afdf5e' "$future_human_gate_recovery_migration" || fail 'HumanGate result artifact identity is not exact'
	grep -Fq 'inference_tokens = 10430' "$future_human_gate_recovery_migration" || fail 'HumanGate recovery token audit is absent'
	grep -Fq "prior_error_code = 'submit_failed'" "$future_human_gate_recovery_migration" || fail 'HumanGate validation failure is not preserved'
	grep -Fq 'RecoverDCPFutureArbiterExactHumanGate' backend/internal/storage/sqlite/store/dcp_future_arbiter_store.go || fail 'HumanGate result recovery is not model-free and transactional'
	...
	[[ -s "$future_arbiter_schema_recovery_migration" ]] || fail 'ordinary-card arbiter schema recovery migration is absent'
	grep -Fq 'dcp-future-arbiter-141e3d64af9568aea9ea1fb6835045060dfd566bc3b21d50ff6f3f90f3f67a52' "$future_arbiter_schema_recovery_migration" || fail 'ordinary-card arbiter failed generation identity is not exact'
	grep -Fq 'provider_inference_tokens    INTEGER NOT NULL CHECK (provider_inference_tokens = 0)' "$future_arbiter_schema_recovery_migration" || fail 'ordinary-card arbiter recovery does not prove zero inference tokens'
	grep -Fq 'OLD.status = '\''authorized'\'' AND NEW.status = '\''consumed'\''' "$future_arbiter_schema_recovery_migration" || fail 'ordinary-card arbiter recovery is not one-way immutable'
	[[ -s "$future_arbiter_result_recovery_migration" ]] || fail 'ordinary-card arbiter result recovery migration is absent'
	grep -Fq 'inference_tokens         INTEGER NOT NULL CHECK (inference_tokens = 10569)' "$future_arbiter_result_recovery_migration" || fail 'ordinary-card exact result token audit is absent'
	grep -Fq "OLD.status = 'pending' AND NEW.status IN ('applied', 'failed')" "$future_arbiter_result_recovery_migration" || fail 'ordinary-card result recovery is not one-way immutable'
	[[ -s "$future_repair_target_recovery_migration" ]] || fail 'ordinary-card repair target recovery migration is absent'
	grep -Fq "authority = 'rearm_same_prelaunch_repair_action_zero_new_model_authority'" "$future_repair_target_recovery_migration" || fail 'ordinary-card repair target recovery expands model authority'
	grep -Fq "action.slot = 0 AND action.launch_id = ''" "$future_repair_target_recovery_migration" || fail 'ordinary-card repair target recovery did not stop before the model fence'
	[[ -s "$future_repair_ci_recovery_migration" ]] || fail 'ordinary-card repair CI snapshot recovery migration is absent'
	grep -Fq "authority = 'bind_exact_green_repair_head_and_queue_one_fresh_reviewer'" "$future_repair_ci_recovery_migration" || fail 'ordinary-card repair CI recovery expands model authority'
	grep -Fq "AND (SELECT COUNT(*) FROM pr_checks current_check" "$future_repair_ci_recovery_migration" || fail 'ordinary-card repair CI recovery does not bind one exact current-head check'
	grep -Fq "'reviewer'," "$future_repair_ci_recovery_migration" || fail 'ordinary-card repair CI recovery does not queue the fresh reviewer'
	grep -Fq 'func (v ReviewRepositoryValidator) ValidateContinuation' backend/internal/service/dcptask/review_repository.go || fail 'ordinary-card continuation target proof is absent'
	grep -Fq 'merge-base", "--is-ancestor", head, originMain' backend/internal/service/dcptask/review_repository.go || fail 'ordinary-card continuation can accept divergent main'
	grep -Fq 'ResolveSessionHandle' backend/internal/dcpterminalmerge/future_arbiter_launcher.go || fail 'ordinary-card arbiter does not validate the adapter physical handle'
	grep -Fq '"not": true, "const": true, "uniqueItems": true' backend/internal/dcpterminalmerge/future_arbiter_protocol.go || fail 'ordinary-card arbiter provider compatibility fence is incomplete'
	! sed -n '/func FutureArbiterDecisionJSONSchema/,/func validateFutureArbiterResponseSchema/p' backend/internal/dcpterminalmerge/future_arbiter_protocol.go | grep -Eq '"(const|uniqueItems|\\$schema)"[[:space:]]*:' || fail 'ordinary-card arbiter generated schema retains an unsupported keyword'
	...
	grep -Fq 'session.dcpArbiterStatus === "human_gate"' frontend/src/renderer/lib/session-presentation.ts || fail 'terminal Human Gate does not override stale stock failure through the shared projection'
	grep -Fq 'statusLabelKey: "status.human_gate"' frontend/src/renderer/lib/session-presentation.ts || fail 'terminal Human Gate lacks a decision-specific primary label'
	grep -Fq 'policyPhase: "arbiter"' frontend/src/renderer/lib/session-presentation.ts || fail 'automatic arbiter work lacks a shared typed presentation phase'
	grep -Fq 'laneSection: "arbiter"' frontend/src/renderer/lib/session-presentation.ts || fail 'automatic arbiter work lacks a distinct subsection projection'
	grep -Fq 'visualStatus("arbiter", "bg-status-arbiter"' frontend/src/renderer/lib/session-presentation.ts || fail 'automatic arbiter work lacks the dedicated purple projection'
	grep -Fq 'ReviewArbiterLaneColumn' frontend/src/renderer/components/SessionsBoard.tsx || fail 'the stock third column is not a shared review/arbiter lane'
	grep -Fq -- '--color-status-arbiter: #c084fc;' frontend/src/styles/tokens.css || fail 'dark-theme arbiter contrast token is absent'
	grep -Fq -- '--color-status-arbiter: #7e22ce;' frontend/src/styles/tokens.css || fail 'light-theme arbiter contrast token is absent'
...
unexpected_paths="$(printf '%s\n' "$unexpected_paths" | grep -Ev '^(backend/internal/cli/dcp\.go|backend/internal/domain/(dcp_lab_policy|dcp_wbc_marker_test)\.go|backend/internal/service/dcptask/(policy|review_repository)\.go|backend/internal/storage/sqlite/gen/dcp_lab_policy\.sql\.go|backend/internal/storage/sqlite/migrations/006(7_dcp_review_lab_happy_path_v1|8_dcp_policy_provider_pending_recovery)\.sql|backend/internal/storage/sqlite/policy_provider_pending_migration_test\.go|backend/internal/storage/sqlite/queries/dcp_lab_policy\.sql|backend/internal/storage/sqlite/store/dcp_lab_policy_store(_test)?\.go)$' || true)"
...
unexpected_paths="$(printf '%s\n' "$unexpected_paths" | grep -Ev '^(frontend/src/renderer/hooks/useWorkspaceQuery(\.test)?\.tsx?|frontend/src/renderer/styles\.css|frontend/src/styles/tokens\.css)$' || true)"
