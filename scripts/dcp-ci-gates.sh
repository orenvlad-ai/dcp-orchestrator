...
source_gates() {
	...
	[[ -s "$dcp_v2_stage6_native_shell_migration" ]] || fail 'exact Stage 6 native-shell migration is absent'
	grep -Fq "task_id = 'dcp-v2-twin-canary-v1'" "$dcp_v2_stage6_native_shell_migration" || fail 'Stage 6 native-shell task identity is not exact'
	grep -Fq "target = 'dcp-wbc-integration-lab'" "$dcp_v2_stage6_native_shell_migration" || fail 'Stage 6 native-shell target identity is not exact'
	[[ -s "$dcp_v2_direct_model_authority_migration" ]] || fail 'DCP v2 direct-model migration is absent'
	grep -Fq 'CREATE TABLE dcp_v2_model_runtime' "$dcp_v2_direct_model_authority_migration" || fail 'DCP v2 direct runtime authority is absent'
	grep -Fq 'CREATE TABLE dcp_v2_model_terminal_receipt' "$dcp_v2_direct_model_authority_migration" || fail 'DCP v2 terminal receipt authority is absent'
	grep -Fq 'CREATE TABLE dcp_v2_stage6_worker_adoption_v1' "$dcp_v2_direct_model_authority_migration" || fail 'DCP v2 exact Worker adoption evidence is absent'
	...
	grep -Fq 'stage6-direct-adopt' backend/internal/cli/dcp_v2_adoption.go || fail 'stopped exact Stage 6 adoption command is absent'
	grep -Fq 'func (s *TwinService) ReportDirectProcessExit' backend/internal/service/dcpv2/direct_model.go || fail 'direct DCP v2 terminal ingestion is absent'
	grep -Fq 'DCP_POLICY_V2_AUTHORITY_REMOVED' backend/internal/service/dcptask/service.go || fail 'legacy DCP-v2 lifecycle entry is not deactivated'
	! grep -ERq 'PolicyV2Bridge|recoverStage6NativeShell|stage6-recovery-preflight' backend/internal || fail 'legacy DCP-v2 second-authority bridge remains reachable'
	...
	[[ -s "$wbc_readmission_admission_recovery_migration" ]] || fail 'WBC readmission admission recovery migration is absent'
	grep -Fq "prior_error_code = 'admission_identity_drift'" "$wbc_readmission_admission_recovery_migration" || fail 'WBC admission false incident is not preserved'
	grep -Fq "action.sequence = 73" "$wbc_readmission_admission_recovery_migration" || fail 'WBC admission recovery reviewer action is not exact'
	...
	grep -Fq "generation.status = 'admitted'" "$wbc_readmission_waiting_recovery_migration" || fail 'WBC waiting recovery lacks admitted-generation evidence'
	grep -Fq 'prior_admission_sequence INTEGER NOT NULL CHECK (prior_admission_sequence = 32)' "$wbc_readmission_waiting_recovery_migration" || fail 'WBC waiting recovery admission is not exact'
	grep -Fq 'resume_exact_bound_wbc_readmission_admission_zero_new_model_or_release_authority' "$wbc_readmission_waiting_recovery_migration" || fail 'WBC waiting recovery authority is not bounded'
	! grep -Eq 'INSERT INTO (dcp_model_action|dcp_review_lab_admission|dcp_wbc_readmission_generation)' "$wbc_readmission_waiting_recovery_migration" || fail 'WBC waiting recovery creates duplicate activity'
	grep -Fq 'boundAdmission := generation.Status == domain.DCPWBCReadmissionAdmitted' backend/internal/dcpterminalmerge/merge.go || fail 'WBC reviewed-to-admitted shell binding is absent'
	...
	[[ -s "$dcp_v2_core_migration" ]] || fail 'DCP v2 Stage 4 additive migration is absent'
	grep -Fq "control_plane_commit  TEXT NOT NULL CHECK (control_plane_commit = '$dcp_v2_control_plane_commit')" "$dcp_v2_core_migration" || fail 'DCP v2 source authority is not exact'
	for v2_table in task revision command action admission incident external_event result; do
		grep -Fq "CREATE TABLE dcp_v2_$v2_table" "$dcp_v2_core_migration" || fail "DCP v2 $v2_table table is absent"
	done
	...
	grep -Fq '0084 DCP v2 Stage 4 core is forward-only' "$dcp_v2_core_migration" || fail 'DCP v2 migration is not forward-only'
	grep -Fq 'func (k DCPV2CommandKind) AllowsTransition' backend/internal/domain/dcp_v2.go || fail 'DCP v2 typed transition law is absent'
	grep -Fq 'func (s *Store) TransitionDCPV2' backend/internal/storage/sqlite/store/dcp_v2_store.go || fail 'DCP v2 atomic state/command store is absent'
	grep -Fq 'func (s *Store) RecoverDCPV2CommandLease' backend/internal/storage/sqlite/store/dcp_v2_store.go || fail 'DCP v2 crash lease recovery is absent'
	grep -Fq "effect_fence = '' AND recovery_generation" backend/internal/storage/sqlite/queries/dcp_v2.sql || fail 'DCP v2 can retry a fenced external effect'
	grep -Fq 'func (e *Engine) Startup' backend/internal/service/dcpv2/engine.go || fail 'DCP v2 startup drain is absent'
	...
	grep -Fq 'getSessionAccessibilityStatus(session)' frontend/src/renderer/lib/session-presentation.test.ts || fail 'DCP v2 projection lacks accessibility proof'
