...
source_gates() {
	...
	printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' "$terminal_merge_migration" "$admission_migration" "$recovered_incident_migration" "$arbiter_migration" "$arbiter_prelaunch_recovery_migration" "$arbiter_schema_recovery_migration" "$arbiter_successor_migration" "$arbiter_successor_validation_recovery_migration" "$fresh_worker_recovery_migration" "$fresh_worker_preflight_recovery_migration" "$model_free_rebase_migration" "$provider_base_correction_migration" "$cold_start_recovery_migration" "$cold_start_tool_path_recovery_migration" "$cold_start_auto_merge_recovery_migration" "$rebase_head_finalization_migration" > "${TMPDIR:-/tmp}/dcp-authorized-migrations.$$"
...
unexpected_paths="$(printf '%s\n' "$unexpected_paths" | grep -Ev '^(backend/internal/storage/sqlite/cold_start_auto_merge_migration_test\.go|backend/internal/storage/sqlite/migrations/0063_dcp_card12_cold_start_auto_merge_recovery\.sql|backend/internal/dcpterminalmerge/rebase_head_finalization_(engine|executor|test)\.go|backend/internal/domain/dcp_card12_rebase_head_finalization\.go|backend/internal/storage/sqlite/(rebase_head_finalization_migration_test\.go|migrations/0064_dcp_card12_rebase_head_finalization\.sql|queries/dcp_card12_rebase_head_finalization\.sql|gen/dcp_card12_rebase_head_finalization\.sql\.go|store/dcp_card12_rebase_head_finalization_store(_test)?\.go))$' || true)"
